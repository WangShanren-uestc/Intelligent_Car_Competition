...
uint8 *imgbuff = (uint8 *)((uint8 *)&sample_image_buff);     //图像地址
uint8 *imgbuff1 = (uint8 *)((uint8 *)&sample_image_buff1);     //图像地址
...
//    size_of_int = sizeof(int);
    ...
    while(1)
    {
		          ...
		          *(imgbuff1+CAMERA_SIZE) = *(imgbuff1);
		          img_extract(ImageBuf, imgbuff1+1,CAMERA_SIZE);
                          ...
                          *(imgbuff+CAMERA_SIZE) = *(imgbuff);
                          ...
                          frameCnt++;//已处理帧计数
                          if(sampleframeCnt-frameCnt){//若丢帧
		...
		//图像解压及处理
                    ...
                    if(nrf_tx_state() != NRF_TXING && sampleframeCnt%4==0&&NRF_on){  //发送NMB
...
//    port_init(PTC16, ALT1 | IRQ_RISING ); //HREF
  ...
  if(NRF_on){
      ...
      while(i--)
      {
          nrf_msg_tx(COM_RETRAN,tx_buff);                              //发送多个 空 命令过去，用于清空 接收端的缓存数据
      }
  }   
    key_init(KEY_MAX);
